...
for rdb in dbConnections:
  ...
  #   #       Column.foreign("int_relation_id").reference("id").on("IntRelation").onDelete(SET_NULL).change(),
  #   #       Column.strForeign("str_relation_id").reference("uuid").on("StrRelation").onDelete(SET_NULL).change()
  #   #     ]),
  #   #     table("TypeUnique", [
  #   #       Column.increments("id").change(),
