...
when defined(unix):
  when defined(macosx):
    const lib = "(libmysqlclient|libmariadb)(|.21|.20|.19|.18|.17|.16|.15).dylib"
  else:
    const osName = getOsName()
    when osName == "alpine":
      const lib = "(libmysqlclient|libmariadb).so(|.3)"
    else:
      const lib = "(libmysqlclient|libmariadb).so(|.21|.20|.19|.18|.17|.16|.15)"
...
# # {.deprecated: "Enum_server_command".}
...
# type
#   Enum_shutdown_level* {.deprecated.} = enum
...
#   Enum_cursor_type* {.deprecated.} = enum    # options for mysql_set_option
#     CURSOR_TYPE_NO_CURSOR = 0, CURSOR_TYPE_READ_ONLY = 1,
#     CURSOR_TYPE_FOR_UPDATE = 2, CURSOR_TYPE_SCROLLABLE = 4
#   Enum_mysql_set_option* {.deprecated.} = enum
...
#   Sockaddr*{.deprecated, final.} = object  # undefined structure
...
#   UDF_ARGS* {.deprecated.} = St_udf_args
...
#   UDF_INIT* {.deprecated.} = St_udf_init
...
#   #   skip_result*: proc (para1: Pst_mysql_bind, para2: PFIELD, row: PPByte)
  ...
  U* {.final, union, pure.} = object # union for row_ptr and indicator
#     row_ptr*: ptr byte  # for the current data position
#     indicator*: cstring

  St_mysql_bind*{.final.} = object
    length*: ptr culong  # output length pointer
    is_null*: Pmy_bool   # Pointer to null indicator
    buffer*: pointer     # buffer to get/put data
    error*: Pmy_bool     # set this if you want to track data truncations happened during fetch
    u*: U
    store_param_func*: proc (net: PNET, param: ptr St_mysql_bind){.cdecl.}
    fetch_result*: proc (param: ptr St_mysql_bind, field: PFIELD, row: PPByte){.cdecl.}
    skip_result*: proc (param: ptr St_mysql_bind, field: PFIELD, row: PPByte){.cdecl.}
    buffer_length*: culong  # buffer length, must be set when fetching str/binary
    offset*: culong  # offset position for char/binary fetch
    length_value*: culong  # Used if length is 0
    flags*: cuint   # special flags, e.g. for dummy bind
    pack_length*: cuint  # Internal length for packed data
    buffer_type*: Enum_field_types  # buffer type
    error_value*: my_bool  # used if error is 0
    is_unsigned*: my_bool  # set if integer type is unsigned
