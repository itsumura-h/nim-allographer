# nim c -r --threads:off -d:reset example/mariadb/benchmark.nim
...
proc mariaBenchmarkScenario(rdb: MariadbConnections) {.async.} =
  rdb.create(
    table("World", [
  ...
  await timeProcess("mariadb benchmark", benchUpdate)
  await timeProcess("mariadb benchmark prepared cold", benchUpdatePreparedCold)
  await timeProcess("mariadb benchmark prepared warm", benchUpdatePreparedWarm)


proc main() =
  let rdb = dbOpen(MariaDB, mariaUrl, maxConnections, timeout, shouldDisplayLog=shouldDisplayLog)
  mariaBenchmarkScenario(rdb).waitFor
